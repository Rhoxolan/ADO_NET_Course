...
namespace _2022._07._08_PW
{
    public partial class Form1 : Form
    {
        ...
        {
            using (NpgsqlConnection conn = new(connStr))
            {
                try
                {
                    conn.Open();
        ...
        {
            using (NpgsqlConnection connection = new(connStr)) //Привязываем в comboBox1 типы канцтоваров (StationeryTypes)
            {
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            using (NpgsqlConnection connection = new(connStr))
            {
                ...
                NpgsqlCommand command = new(queryStr, connection);
                NpgsqlParameter titleParam = command.Parameters.Add("@name", NpgsqlDbType.Text, 1000);
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            using (NpgsqlConnection connection = new(connStr))
            {
                ...
                {
                    connection.Open();
                    NpgsqlCommand cmd = new("ShowAVGSellingsOnYear", connection);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@year", NpgsqlDbType.Integer).Value = Convert.ToInt32(textBox2.Text);
                    NpgsqlParameter outputParam = new("@avgPrice", SqlDbType.Int);
